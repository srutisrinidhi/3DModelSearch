"""Ablation: multi-view image fusion via MLP.

Stores one embedding per model: the 6 view embeddings pooled by the trained multi-view MLP
model. ChromaDB collection ``image_embeddings`` (inner-product metric). Requires a trained
MLP checkpoint (see ``multimodal_models/train_multiview_image_mlp.py``).

Example:
    python create_multiview_mlp_db.py \
        --models_csv out/models_data.csv \
        --model_path models/multiview_image_mlp.pt \
        --persist_directory databases/multiview_image_mlp
"""
...
def main():
    ...
    added = 0
    for uid, embs in tqdm(image_embeddings.items(), desc="Building multiview-MLP DB"):
        vec = get_multiview_image_mlp_embeddings(embs, model, device=device).squeeze(0)
