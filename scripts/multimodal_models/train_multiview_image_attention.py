"""Train the attention-based multi-view image fusion model.

The model (``MultiViewImage``) uses a learnable query that cross-attends over the 6 per-view
image embeddings, followed by an MLP with residual connections, to produce a single 768-d
vector. Training is self-supervised over the views; it only needs the per-model image
embeddings from the mega CSV produced by ``generate_embeddings.py``.

Example:
    python train_multiview_image_attention.py \
        --models_csv out/models_data.csv \
        --model_output_path models/multiview_image_attention.pt
"""
...
class MultiViewImage(nn.Module):
    ...
    def forward(self, embeddings):
        ...
        attn_output, attn_weights = self.attn(query, embeddings, embeddings)  # (B, 1, D)
        attn_output = attn_output + query  # residual connection
        attn_output = self.norm1(attn_output)

        mlp_output = self.mlp(attn_output)
        final_output = self.norm2(mlp_output + attn_output)
...
def train_model(model, train_loader, val_loader, optimizer, num_epochs=100,
    ...
    if loss_plot:
        _plot_losses(train_losses, val_losses, num_epochs,
                     "Training and Validation Loss for Attention based model", loss_plot)
...
def load_model(path, device=None):
    """Load a trained MultiViewImage attention checkpoint (state_dict) onto ``device``."""
...
def get_multiview_image_attention_embeddings(image_embeddings, model, device=None):
...
def main():
    parser = argparse.ArgumentParser(description="Train the attention multi-view image fusion model.")
    parser.add_argument("--models_csv", required=True,
                        help="Mega CSV from generate_embeddings.py.")
    parser.add_argument("--model_output_path", default="models/multiview_image_attention.pt",
